...
Phase 1 - PREDICT & MONITOR (core focus): AI reads satellite/drone imagery
          indicators, flags risk with an explainable factor breakdown, shows
          it on a real satellite map, cross-references past incidents nearby,
          and fires a siren + browser notification on HIGH/CRITICAL.
...
from utils.alert_utils import get_siren_bytes, browser_notification, sos_button_css
from utils.map_utils import build_full_map, build_history_map
...
st.markdown("""
<style>
    .resq-title {font-size: 42px; font-weight: 900; letter-spacing: 2px; color: #fff; margin-bottom: 0;}
    .resq-sub {color:#8b93a7; font-size:14px; margin-top:-6px;}
    .ticker {background:#14171f; border-left:4px solid #ff1f1f; padding:8px 14px; border-radius:6px;
             color:#ff9d9d; font-family:monospace; font-size:13px;}
</style>
...
def nearest_incident(lat, lon, history_df):
    d = history_df.copy()
    d["dist_km"] = d.apply(lambda r: haversine_km(lat, lon, r["lat"], r["lon"]), axis=1)
...
if "play_siren" not in st.session_state:
    st.session_state.play_siren = False
...
with st.sidebar:
    ...
    if st.button("🔊 Test Alert Siren", use_container_width=True):
        st.session_state.play_siren = True
    st.markdown("---")
    page = st.radio(
        "NAVIGATE",
    ...
    st.markdown('<a href="tel:112" class="sos-btn">🆘 SOS · CALL 112</a>', unsafe_allow_html=True)
...
with h2:
    st.markdown(f"<div class='ticker'>🔴 LIVE &nbsp;|&nbsp; {critical_count} CRITICAL &nbsp;|&nbsp; "
                f"{high_count} HIGH &nbsp;|&nbsp; {len(MONITORED_REGIONS)} zones monitored</div>", unsafe_allow_html=True)

if st.session_state.play_siren:
    siren = get_siren_bytes()
    if siren:
...
if page.startswith("🛰️"):
    ...
    st.caption("Catching a disaster **before** it fully unfolds — continuously reading satellite "
               "and drone feeds across monitored zones and scoring risk from live indicators.")

    c1, c2 = st.columns([1, 2])
    with c1:
        scan_clicked = st.button("🔄 Run New AI Scan", use_container_width=True, type="primary")
    with c2:
        st.caption("Each scan re-analyzes every monitored zone's latest satellite/drone indicators "
                   "(rainfall anomaly, ground deformation, glacial melt rate, storm surge risk, etc. — "
                   "the specific factors depend on each region's geography).")
    ...
    if critical_now:
        ...
        &nbsp;·&nbsp; radius <b>{top['affected_radius_km']} km</b><br>
        <small>{top['notes']}</small>
        </div>
        """, unsafe_allow_html=True)

    st.markdown("### Live Satellite Monitoring")
    try:
        ...
        st.error("streamlit-folium is not installed — add it to requirements.txt to see the live map.")

    st.markdown("### Zone-by-Zone Risk — with AI Reasoning")
    for p in predictions[:len(MONITORED_REGIONS)]:
        near = nearest_incident(p["lat"], p["lon"], history_df)
        factor_rows = "".join(
            f"<div class='factor-row'><span style='width:150px'>{name}</span>"
            f"<div class='factor-bar-bg'><div class='factor-bar-fill' style='width:{score}%;'></div></div>"
            f"<span style='width:34px;text-align:right'>{score}</span></div>"
            for name, score in p["factors"].items()
        )
        st.markdown(f"""
        <div class='resq-card {p['risk_level'].lower()}'>
        <b>{p['region']}</b> &nbsp; <span class='risk-badge-{p['risk_level'].lower()}'>{p['risk_level']}</span>
        &nbsp;·&nbsp; {p['disaster_type']} &nbsp;·&nbsp; score {p['risk_score']}/100 &nbsp;·&nbsp; confidence {p['confidence']}%
        <br><small>Source: {p['source']} &nbsp;|&nbsp; Scanned: {p['timestamp']}</small>
        <div style='margin-top:10px;'>{factor_rows}</div>
        <div class='prev-incident'>📜 Nearest past incident: <b>{near['name']}</b> ({near['date']}, {int(near['dist_km'])} km away) —
        {near['deaths']} deaths, {near['affected_people']:,} affected.</div>
        </div>
        """, unsafe_allow_html=True)
    ...
    with a1:
        df_type = df_pred["disaster_type"].value_counts().reset_index()
        ...
        st.caption("What kind of disaster each monitored zone's imagery currently points to.")
    with a2:
        df_risk = df_pred["risk_level"].value_counts().reindex(["LOW", "MODERATE", "HIGH", "CRITICAL"]).fillna(0).reset_index()
        df_risk.columns = ["Risk", "Count"]
        color_map = {"CRITICAL": "#ff1f1f", "HIGH": "#ff6a00", "MODERATE": "#ffcc00", "LOW": "#2ecc71"}
        ...
        st.caption("How many of the 6 monitored zones fall in each risk band right now.")
    with a3:
        avg_score = df_pred["risk_score"].mean()
        fig_gauge = go.Figure(go.Indicator(
            mode="gauge+number", value=avg_score,
            title={"text": "Avg. Regional Risk Score"},
            gauge={"axis": {"range": [0, 100], "tickvals": [0, 30, 55, 80, 100]},
                   "bar": {"color": "#ff1f1f" if avg_score > 55 else "#2ecc71"},
                   "steps": [{"range": [0, 30], "color": "#123"}, {"range": [30, 55], "color": "#332"},
                             {"range": [55, 80], "color": "#421"}, {"range": [80, 100], "color": "#500"}]}))
        fig_gauge.update_layout(margin=dict(t=40, b=0, l=20, r=20), height=300)
        st.plotly_chart(fig_gauge, use_container_width=True)
        st.caption("Average of all 6 zones' risk scores (0=safe, 100=maximum risk).")
    ...
    if st.button("📡 Dispatch alert to authorities + nearby resources"):
        ...
        st.dataframe(nearest[["name", "type", "region", "contact", "dist_km"]]
                     .rename(columns={"dist_km": "distance_km"}).round({"distance_km": 1}),
...
# ==========================================================================
    ...
    st.caption("Once a zone is flagged, people on the ground add real detail: what's actually happening, "
               "how many are affected, and what help is needed.")
    ...
    with st.form("ground_report_form", clear_on_submit=True):
        ...
        if submitted:
            st.session_state.ground_reports.append({
                ...
                "severity": r_severity, "description": r_desc, "has_photo": r_photo is not None,
            })
            st.success("Report submitted — added to the live feed below.")
    ...
    if st.session_state.ground_reports:
        ...
        b1, b2 = st.columns(2)
        with b1:
            st.plotly_chart(px.pie(rep_df, names="type", title="Reports by Disaster Type", hole=0.45), use_container_width=True)
        with b2:
            fig2 = px.bar(rep_df, x="location", y="people_affected", color="severity",
                          color_discrete_map={"Low": "#2ecc71", "Moderate": "#ffcc00", "High": "#ff6a00", "Critical": "#ff1f1f"},
...
# ==========================================================================
    ...
    r1, r2 = st.columns(2)
    with r1:
        st.plotly_chart(px.pie(filtered, names="type", title="Resource Mix", hole=0.45), use_container_width=True)
    with r2:
        st.plotly_chart(px.bar(filtered, x="region", y="capacity", color="type", title="Capacity by Region"), use_container_width=True)
    ...
    if active:
        for p in active:
            f = filtered.copy()
            f["dist_km"] = f.apply(lambda r: haversine_km(p["lat"], p["lon"], r["lat"], r["lon"]), axis=1)
            nearest = f.sort_values("dist_km").head(3)
        ...
        st.info("No HIGH/CRITICAL zones right now — resource matching will appear automatically when one is detected.")
...
# ==========================================================================
    ...
    country_filter = st.multiselect("Country", history_df["country"].unique().tolist(), default=history_df["country"].unique().tolist())
    type_filter2 = st.multiselect("Type", history_df["type"].unique().tolist(), default=history_df["type"].unique().tolist())
    ...
    try:
        from streamlit_folium import st_folium
        hm = build_history_map(hist_filtered)
        st_folium(hm, use_container_width=True, height=420, returned_objects=[])
    except ModuleNotFoundError:
        st.error("streamlit-folium is not installed — add it to requirements.txt to see the historical map.")

    c1, c2 = st.columns(2)
    with c1:
        fig = px.bar(hist_filtered.sort_values("deaths", ascending=False), x="name", y="deaths", color="type", title="Deaths by Incident")
        fig.update_layout(xaxis_tickangle=-35)
        st.plotly_chart(fig, use_container_width=True)
    with c2:
        st.plotly_chart(px.pie(hist_filtered, names="type", title="Incidents by Type", hole=0.45), use_container_width=True)
